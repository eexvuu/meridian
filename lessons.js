...
export function evolveThresholds(perfData, config) {
  ...
  // Skip entirely when current is null — caller has explicitly disabled the cap.
  {
    ...
    const current    = config.screening.maxVolatilityForDeploy;
    ...
    if (isFiniteNum(current)) {
      if (loserVols.length >= 2) {
        ...
        if (loserP25 < current) {
          ...
          if (rounded < current) {
            changes.maxVolatilityForDeploy = rounded;
            rationale.maxVolatilityForDeploy = `Losers clustered at volatility ~${loserP25.toFixed(1)} — tightened from ${current} → ${rounded}`;
          }
        }
        ...
        if (winnerP75 > current * 1.1) {
          ...
          if (rounded > current) {
            changes.maxVolatilityForDeploy = rounded;
            rationale.maxVolatilityForDeploy = `All ${winners.length} positions profitable — loosened from ${current} → ${rounded}`;
          }
        }
      }
    }
  ...
  if (changes.maxVolatilityForDeploy != null) s.maxVolatilityForDeploy = changes.maxVolatilityForDeploy;
